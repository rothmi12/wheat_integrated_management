...
```{r, Acres planted}
planted.df <- read.csv2("planted_acres_and_yield.csv", 
                        sep = ",", header = T)
planted.df$Year
...
ggplot(planted.df, aes(x=Year)) +
  geom_smooth(aes(y = WHEAT_HECTARES_PLANTED), 
            colour = "black", linetype = 1, size = 2) +
  geom_abline(slope = -0.3281, intercept = 681.9796, 
              linetype = 2, colour = "black") +
  geom_text(x=2017, y=16.5, label="y = -0.33x + 682.0 \n P < 0.001",
            size = 8) +
  geom_smooth(aes(y = WHEAT_TONNES_PRODUCED/2), 
            colour = "grey60", linetype = 1, size = 2) +
  geom_abline(slope = (-0.09773/2), intercept = (252.85130/2),
  ...
  scale_y_continuous(sec.axis = sec_axis(~.*2, name = "Wheat yield (millions of tonnes)")) +
...
confint(plant.lm, level = 0.95)

yield.lm <- lm(WHEAT_TONNES_PRODUCED~Year, planted.df)
summary(yield.lm)
yield.lm$coefficients
confint(yield.lm, level = 0.95)

```

```{r, get means}
colnames(master.df)
get_mean <- function(var, man) {
  temp.df <- with(master.df, master.df[variety == as.character(var) & 
                                       management == as.character(man),])
  mean(na.omit(temp.df$Stripe_Rust_DX_beta))
}
get_mean("735","Current")
get_mean("735","MidLevel")
get_mean("735","HighLevel")
levels(master.df$variety)

get_mean2 <- function(man) {
  temp.df <- master.df[master.df$management == as.character(man),]
  mean(na.omit(temp.df$Yield_bushels_acre))
}
get_mean2("Current")
get_mean2("MidLevel")
get_mean2("HighLevel")
